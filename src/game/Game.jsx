...
const CURATED = [
  { lat: 48.85837, lng: 2.294481, panoId: null },
  ...
  { lat: 41.89021, lng: 12.492231, panoId: null },
...
async function pickStreetViewLocation(google, mode, country) {
  ...
  if (mode === "country" && country) {
    try {
      ...
      console.warn("Country geocode failed, falling back");
    }
  }
...
function GuessMap({ googleReady, guess, answer, onGuess }) {
  ...
  React.useEffect(() => {
    ...
    mapRef.current = map;
    // Decouple from visible Street View
    try {
      const hiddenDiv = document.createElement("div");
      hiddenDiv.style.width = "0px";
      hiddenDiv.style.height = "0px";
      const dummy = new google.maps.StreetViewPanorama(hiddenDiv, { visible: false });
      map.setStreetView(dummy);
    } catch {}
    map.addListener("click", (e) => onGuess && onGuess([e.latLng.lat(), e.latLng.lng()]));
  }, [googleReady, onGuess]);
  ...
  React.useEffect(() => {
    ...
    if (guess) {
      ...
      if (!gRef.current) {
        if (hasAdv) {
          ...
          Object.assign(el.style, { width: "14px", height: "14px", borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 0 2px rgba(34,197,94,0.35)" });
    ...
    } else if (gRef.current) { if (hasAdv) gRef.current.map = null; else gRef.current.setMap(null); gRef.current = null; }
    // Answer marker
    if (answer) {
      ...
      if (!aRef.current) {
        if (hasAdv) {
          const el = document.createElement("div");
          Object.assign(el.style, { width: "14px", height: "14px", borderRadius: "50%", background: "#3b82f6", boxShadow: "0 0 0 2px rgba(59,130,246,0.35)" });
    ...
    } else if (aRef.current) { if (hasAdv) aRef.current.map = null; else aRef.current.setMap(null); aRef.current = null; }
    // Line + fit
    if (answer && guess) {
...
export default function Game({ user }) {
  ...
  async function saveFavourite() {
    ...
    if (!answer) return;
    const label =
      prompt("Label for this favourite:", `Round ${round} — ${answer.lat.toFixed(3)}, ${answer.lng.toFixed(3)}`) ||
      `Round ${round}`;
    try {
      ...
      console.error(e);
      alert("Failed to save favourite. Check rules & permissions.");
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 h-[50vh] lg:h-[70vh] rounded-2xl overflow-hidden shadow-xl ring-1 ring-white/10">
          ...
          {error && !loading && (
            <div className="w-full h-full grid place-items-center p-6 text-center bg-slate-900/60">
              <div className="space-y-2">
                ...
                <button onClick={startNewRoundInternal} className="px-4 py-2 bg-slate-700 rounded-xl hover:bg-slate-600" disabled={picking}>
                  Try again
                </button>
              </div>
            </div>
          ...
          {googleReady && !loading && !error && answer && (
            <StreetViewPane googleReady={googleReady} panoLatLng={{ lat: answer.lat, lng: answer.lng }} />
          )}
        </div>
            ...
            <div className="flex gap-2">
              ...
              {round >= maxRounds && (
                <button
                  onClick={saveScore}
                  disabled={!fbReady || !user}
                  className="px-5 py-2 rounded-xl bg-pink-600 hover:bg-pink-500 disabled:bg-slate-600 disabled:cursor-not-allowed"
                >
                  Save score
                </button>
              )}
      ...
      {reveal && lastResult && (
        <div className="rounded-2xl bg-slate-900/70 ring-1 ring-white/10 p-4 flex flex-wrap items-center gap-4">
          <div className="text-lg font-semibold">
            Distance: <span className="text-amber-300">{formatKm(lastResult.distanceKm)}</span>
          </div>
          <div className="text-lg font-semibold">
            Points: <span className="text-emerald-300">{lastResult.points}</span>
          </div>
